...
namespace desktop { namespace core { namespace agent {
	...
	, m_iniFileService(std::move(iniFileService))
	{
		...
		auto logFiles = m_iniFileService->get<bool>(documents + "Application.ini", "Log", "Enabled", true);
		...
		if (logFiles)
		{
			...
			auto rotating_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(documents + "Logs\\Application.log", 1024 * 1024 * 10, 1, true);
			std::vector<spdlog::sink_ptr> sinks{ stdout_sink, rotating_sink };

			m_logger = std::make_shared<spdlog::async_logger>("ApplicationLog", sinks.begin(), sinks.end(), spdlog::thread_pool(), spdlog::async_overflow_policy::block);
